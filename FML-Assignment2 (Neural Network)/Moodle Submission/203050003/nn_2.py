...
class Net(object):
	...
	def backward(self, X, y, lamda):
		...
		for i in reversed(range(len(self.weights) - 1)):
			del_W.append((np.matmul(self.A[i].transpose(), delta) + lamda * self.weights[i + 1]))
			del_b.append(np.mean(delta, axis = 0) + lamda * self.biases[i + 1])
			delta = np.matmul(delta, self.weights[i + 1].transpose()) * self.dz[i]

		del_W.append((np.matmul(X.transpose(), delta) + lamda * self.weights[0]))
		del_b.append(np.mean(delta, axis = 0) + lamda * self.biases[0])
		del_W.reverse()
		del_b.reverse()
